...
(function () {
  ...
  var liveRoundListEl = document.getElementById("liveRoundList");
  var liveAllTimeListEl = document.getElementById("liveAllTimeList");
  var lastSavedTextEl = document.getElementById("lastSavedText");
  ...
  // TikTok's CDN photo URLs are hotlink-protected and can silently refuse
  // to load when requested straight from a different site's <img> tag
  // (exactly what this page would otherwise do). Routing them through this
  // server's own /avatar proxy (see server.js) sidesteps that - the proxy
  ...
  // ---- Always-on scoreboard (top 5 this round + top 5 all-time) -----------
  // Sits directly under the board, outside the collapsible panel, so it is
  // visible in normal view and in Full Screen without opening anything.
  function renderCompactList(listEl, list) {
    if (!listEl) return;
    listEl.innerHTML = "";
    ...
    top.forEach(function (entry, idx) {
      ...
      rank.textContent = rankBadge(idx);
      li.appendChild(rank);
      li.appendChild(makeAvatarImg(entry.avatar, entry.uniqueId, entry.name, "sm"));
  ...
  function renderLiveScoreboard(roundList, allTimeList) {
    renderCompactList(liveRoundListEl, roundList);
    renderCompactList(liveAllTimeListEl, allTimeList);
  }

  // ---- Small message bar (reset done, backup restored, ...) ---------------
  var hostNoticeBarEl = document.getElementById("hostNoticeBar");
  var hostNoticeTimer = null;
  function showHostNotice(message, ok) {
    if (!hostNoticeBarEl || !message) return;
    hostNoticeBarEl.textContent = message;
    hostNoticeBarEl.classList.toggle("notice-bad", ok === false);
    hostNoticeBarEl.hidden = false;
    if (hostNoticeTimer) clearTimeout(hostNoticeTimer);
    hostNoticeTimer = setTimeout(function () { hostNoticeBarEl.hidden = true; }, 5000);
  }
  socket.on("hostNotice", function (n) {
    n = n || {};
    showHostNotice(n.message || "", n.ok !== false);
  });

  // ---- Score storage status (Settings) -------------------------------------
  // Tells the host, in plain words, whether points are really safe: cloud
  // storage connected, or only the temporary file Render's free plan erases.
  var storageStatusEl = document.getElementById("storageStatus");
  var storageHintEl = document.getElementById("storageHint");
  function formatClock(ts) {
    try {
      return new Date(ts).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });
    } catch (e) { return ""; }
  }
  function updateStorageUI(st) {
    if (!storageStatusEl || !st) return;
    var cls, text, hint = "";
    if (st.cloudConfigured && st.cloudOk === true && !st.cloudPaused) {
      cls = "storage-ok";
      text = "\u2601\uFE0F Cloud storage connected - scores survive sleeping, restarts and new deploys.";
    } else if (st.cloudConfigured) {
      cls = "storage-warn";
      text = "\u26A0\uFE0F Cloud storage is not reachable right now" + (st.lastError ? " (" + st.lastError + ")" : "") +
        ". Scores are still kept on this server and sync again automatically once it is back.";
    } else {
      cls = "storage-warn";
      text = "\u26A0\uFE0F Cloud storage is not set up. Scores are kept in a temporary file that Render's free plan erases whenever the game sleeps, restarts or is redeployed.";
      hint = "To keep scores permanently, add the two UPSTASH_REDIS_REST_* variables on Render (step by step in README.md) - it is free. Until then, download a backup after big streams.";
    }
    if (st.fileOk === false) text += " Also: this server could not write its local save file.";
    storageStatusEl.className = "storage-status " + cls;
    storageStatusEl.textContent = text + "  (" + (st.players || 0) + " viewer" + (st.players === 1 ? "" : "s") + " on the all-time board)";
    if (storageHintEl) { storageHintEl.textContent = hint; storageHintEl.hidden = !hint; }
    if (lastSavedTextEl) {
      lastSavedTextEl.textContent = st.lastSavedAt ? formatClock(st.lastSavedAt) + (st.cloudConfigured && st.cloudOk === true ? " (cloud)" : " (this server only)") : "(nothing to save yet)";
    }
  }
  ...
  socket.on("state", function (state) {
    ...
    renderLiveScoreboard(state.leaderboard, state.allTimeLeaderboard);
    updateStorageUI(state.storage);
  ...
  socket.on("puzzleSolved", function (data) {
    ...
    renderLiveScoreboard(data.leaderboard, data.allTimeLeaderboard);
    roundIsSolved = true;
  ...
  function triggerNewPuzzle() {
    // A stray tap on the toolbar's refresh button in the middle of a round
    // would wipe everyone's round points - ask first when there are points
    // on the board. (All-time points are never affected.)
    var roundHasPoints = lastLeaderboardData.round && lastLeaderboardData.round.length > 0;
    if (roundHasPoints && !roundIsSolved &&
        !window.confirm("Start a new puzzle now? This round's points will be cleared (everyone keeps their all-time points).")) {
      return;
    }
  ...
  // ---- Reset scores (Settings box + button inside the trophy window) --------
  function confirmReset(kind) {
    var all = kind === "alltime";
    var msg = all
      ? "Reset the ALL-TIME leaderboard for everyone? This cannot be undone. (Tip: download a backup first in Settings.)"
      : "Reset the THIS ROUND leaderboard? Everyone starts again from 0 for this puzzle. All-time points are kept.";
    if (window.confirm(msg)) socket.emit(all ? "host:resetAllTimeScores" : "host:resetRoundScores");
  }
  document.getElementById("resetRoundBtn").addEventListener("click", function () { confirmReset("round"); });
  document.getElementById("resetAllTimeBtn").addEventListener("click", function () { confirmReset("alltime"); });
  document.getElementById("lbResetBtn").addEventListener("click", function () {
    var activeTab = document.querySelector(".lb-modal-tab.active");
    confirmReset(activeTab && activeTab.getAttribute("data-lb-tab") === "alltime" ? "alltime" : "round");
  });

  // ---- Backup restore (the download is a plain link to /api/alltime-backup) --
  var backupRestoreBtnEl = document.getElementById("backupRestoreBtn");
  var backupFileInputEl = document.getElementById("backupFileInput");
  if (backupRestoreBtnEl && backupFileInputEl) {
    backupRestoreBtnEl.addEventListener("click", function () { backupFileInputEl.click(); });
    backupFileInputEl.addEventListener("change", function () {
      var file = backupFileInputEl.files && backupFileInputEl.files[0];
      if (!file) return;
      if (file.size > 4000000) {
        showHostNotice("That file is too big to be a scores backup.", false);
        backupFileInputEl.value = "";
        return;
      }
      var reader = new FileReader();
      reader.onload = function () {
        socket.emit("host:importAllTimeScores", { json: String(reader.result || "") });
        backupFileInputEl.value = "";
      };
      reader.onerror = function () {
        showHostNotice("Could not read that file.", false);
        backupFileInputEl.value = "";
      };
  ...
  // loads. The saved snapshot is only handed to the server if NOBODY has set
  // this server run up yet (state.configured is false) - e.g. right after
  // Render woke it from sleep. That way opening the page on a second phone in
  // the middle of a show can never change the mode or difficulty, and can
  // never clear the puzzle or the points that are on screen. The server makes
  // the final call too, so even two pages loading at the same instant are safe.
  function applyHostDefaultsOnce(state) {
    ...
    // Purely local field - always fine to fill in.
    ...
    socket.emit("host:applyDefaults", {
      ...
      autoNextRound: defaults.autoNextRound,
      autoNextDelaySeconds: defaults.autoNextDelaySeconds,
      botAutoSolveEnabled: defaults.botAutoSolveEnabled
    });
  }
