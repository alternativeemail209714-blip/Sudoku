...
v2.7 changes - "correct guess still says wrong format / some guesses never register":
  - Broadened comment-text extraction to handle array/object-shaped
    comment payloads, not just plain strings.
  - Replaced a de-duplication bug that could (in principle) drop a
    genuinely different, correctly-formatted guess if the connector
    library ever reused an object reference between messages - now keyed
    on the message's own ID instead of object identity.
  - Made the per-message full-payload debug logging opt-in
    (`CHAT_DEBUG_LOG=true`) instead of always-on, so it can't add
    processing lag during a busy live show. See CHANGES.md.

v2.6 changes - "Lost connection (the reconnect attempt failed)" fix:
  - Root cause: `tiktok-live-connector@2.4.4` (this project's previous
    pin) has a confirmed bug where an EulerStream rate-limit response
    (HTTP 429 - a short-term throttle, not your monthly key quota) crashes
    while building its own error object, producing an opaque
    `TypeError: Cannot read properties of undefined (reading
    'retry-after')` instead of a real "you're rate-limited" error. The old
    reconnect logic didn't recognize this and kept retrying every few
    seconds, re-triggering the same limit in a loop - which is exactly
    what "Lost connection (the reconnect attempt failed)" over and over
    was. Fixed upstream in `tiktok-live-connector@2.5.0` (2026-09-16); see
    CHANGES.md for the full writeup, including why the real error message
    was invisible in the Render logs and how that's now fixed too.
