...
// ---------------------------------------------------------------------------
// 3b. ALL-TIME LEADERBOARD PERSISTENCE
//
// Bug this fixes: "points aren't counted in the all-time leaderboard."
// state.allTimeScores (inside createGameState) only ever lived in the
// server's RAM. It was never written anywhere else. That means it was
// correctly incremented on every correct guess (see applyGuess above) for
// as long as the Node process stayed alive - but the INSTANT that process
// restarts for any reason, it comes back as an empty {} and every viewer's
// all-time total is gone, with no error, no log, nothing visibly "broken."
//
// The most common way that happens is Render's free plan: a free web
// service spins down after roughly 15 minutes with no incoming traffic,
// and spins a brand-new process back up on the next request. A host who
// streams once a day (or even just takes a break mid-stream) will come
// back to a completely reset all-time board - which looks exactly like
// "points aren't counted," even though every single point WAS counted
// correctly while the process was running.
//
// The fix: persist state.allTimeScores to a JSON file, reloaded at startup
// and re-saved (debounced, so a burst of guesses doesn't hammer the disk)
// every time someone scores.
//
// HONEST CAVEAT: Render's FREE plan uses an EPHEMERAL filesystem. Anything
// written to disk is wiped on every new deploy, and can also be wiped on
// some restarts, so this file survives ordinary spin-down/spin-up cycles
// but is not a guarantee across redeploys. For all-time scores that
// survive redeploys too, attach a paid Render Persistent Disk and point
// DATA_DIR at its mount path (see the commented-out block in render.yaml).
// Without one, this is a real, meaningful improvement over "always lost,"
// not a 100% guarantee - the same honesty CHANGES.md applies to chat
// delivery applies here to disk persistence.
// ---------------------------------------------------------------------------
...
var saveAllTimeScoresTimer = null;

function loadAllTimeScoresFromDisk() {
  try {
    if (fs.existsSync(ALLTIME_SCORES_FILE)) {
      var raw = fs.readFileSync(ALLTIME_SCORES_FILE, "utf8");
      var parsed = JSON.parse(raw);
      if (parsed && typeof parsed === "object") {
        console.log(
          "[all-time scores] loaded " + Object.keys(parsed).length +
          " player(s) from " + ALLTIME_SCORES_FILE
        );
        return parsed;
      }
    } else {
    ...
    console.warn(
      "[all-time scores] could not load " + ALLTIME_SCORES_FILE +
      " - starting fresh (this run's totals will not include older ones): " +
...
function saveAllTimeScoresNow() {
  try {
    ...
    fs.writeFileSync(ALLTIME_SCORES_FILE, JSON.stringify(game.state.allTimeScores), "utf8");
  } catch (err) {
    console.warn(
      "[all-time scores] could not save to " + ALLTIME_SCORES_FILE + ": " +
      (err && err.message ? err.message : err)
    );
  }
}

// Debounced so a burst of correct guesses (a fast audience, or bot
// auto-solve) triggers one disk write shortly after things settle down,
// not one write per guess.
function saveAllTimeScoresDebounced() {
  if (saveAllTimeScoresTimer) clearTimeout(saveAllTimeScoresTimer);
  saveAllTimeScoresTimer = setTimeout(function () {
    saveAllTimeScoresTimer = null;
    saveAllTimeScoresNow();
  }, SAVE_DEBOUNCE_MS);
}

var game = createGameState();
game.state.allTimeScores = loadAllTimeScoresFromDisk();

// Also save on a normal shutdown (e.g. Render deploying a new version),
// so the last few seconds of debounced-but-not-yet-flushed points aren't
// lost to the timer not having fired yet.
process.on("SIGTERM", function () { saveAllTimeScoresNow(); });
process.on("SIGINT", function () { saveAllTimeScoresNow(); });
...
function processComment(text, uniqueId, nickname, source, avatarUrl) {
  ...
  if (result.status === "correct") saveAllTimeScoresDebounced();
  io.emit("guessResult", {
    uniqueId: uniqueId,
    nickname: nickname,
    avatar: knownAvatar,
